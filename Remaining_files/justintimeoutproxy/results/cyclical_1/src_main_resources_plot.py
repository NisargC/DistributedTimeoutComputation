...
client = 'yclient-7647dd9ddf-wqgrl'
if len(sys.argv) > 1:
    client = sys.argv[1]
lines = []
for i in range(0, 4):
    ...
    l, = (axs.flat)[i].plot_date([], [], linestyle='dashed', marker='None',label='P1 w/ L', color='b', lw=1.5, alpha=.3)
    lines.append(l)
    l, = (axs.flat)[i].plot_date([], [], linestyle='solid', marker='None', label='P1', color='b', lw=1.5)
    lines.append(l)
    l, = (axs.flat)[i].plot_date([], [], linestyle='dashed', marker='None',label='P2 w/ L', color='red', lw=1.5, alpha=.3)
    lines.append(l)
    l, = (axs.flat)[i].plot_date([], [], linestyle='solid', marker='None',label='P2', color='red', lw=1.5)
    lines.append(l)
    l, = (axs.flat)[i].plot_date([], [], label='A', color='black', marker='.', lw=1, alpha=.4)
    lines.append(l)
