...
function string2UTC(str::String, dform)
    date = tryparse(DateTime, str[1:11], dform)
...
function get_model_data(endpoint::String, path::String)
    dform = DateFormat("yyyy-mm-ddTHH:MM:SSZ")
    data = DataFrames.DataFrame(CSV.File("$(@__DIR__)/scenarios/$(path)"))

    #filter out our blacklist
    data = filter(row -> !(row.service_name ∈ ["istio-mixer", "istio-ingressgateway"]), data)

    show(data)
    data[:time_seconds] = data[:time]/1000000000 #map(x -> string2UTC( string(x), dform), data[:time])

    #data[:time_seconds] = string2UTC.( string(data[:time]) , dform)
    #remap
    #data[:mean] = data[:duration]
    ...
    #alldata = alldata[sample(axes(alldata, 1), 1000; replace = false, ordered = true), :]
    #REMAP for ease of use  - TODO argument or preprocess step
#    data[:mean] = data[:max]
