...
function run_predictions(num_samples::Int, endpoint::String, seconds_lookback::Int, seconds_lookforward::Int)

    # load and rescale the dataset
    numPredicted = convert(Int, seconds_lookforward/10) # 20 second intervals
    alldata = @time get_current_data(seconds_lookback, endpoint, numPredicted) #2 hours was too much
    ...
    for subdf in groupby(alldata, :service_name)
        ...
        #TODO remember the average or last value of this endpoint and use that rather than a hardcoded value
        if length(xs_train) < 5
            return 10000
        end
        ...
        for i in 1:num_samples
            ...
            @time (covariance_fn, noise) = inference(xs_train, ys_train, 300, callback)
            ...
            #show(subdf)
            #println("Ys Predicted Transformed")
            #println(pred_ys)
            push!(all_pred_ys,pred_ys)

            #println("Ys Train Transformed")
            #println(ys_train)

            #display(plot(subdf.xs, vcat(subdf[:mean_millis][1:split], pred_ys),  seriestype = :scatter))


        end

        #display(plot_predictions(xs_train, subdf[:mean_millis][1:split], xs_test, all_pred_ys))
        allYs = collect(Iterators.Flatten(all_pred_ys))
        #println("ANSWER SHOULD BE: ", convert(Int, round(StatsBase.quantile(allYs, .99))))
        final =  convert(Int, round(1.5*StatsBase.maximum(allYs)))
        if final < 100
            final = final * 1.5
        end
